...
#include <QDebug>
#include <string>
class multifunc : public QObject
{
public:
    ...
    static QString IntDecToStrHex(int decnum)
    {
      int ys=0;
      int i=0;
      int temp[100];
        ...
        for(int i=2;i<6;i++)
         {
           QByteArray a="";
           a=a+data[i];
           c.append(multifunc::ByteArrayToHexStr(a));

        }
        char* f;
        QByteArray ff=c.toLatin1();
        f=ff.data();
        int data10=0;
        for(int i=0;i<8;i++)
        {
            d=((f[i]>='0'&&f[i]<='9')?f[i]-'0':f[i]-'A'+10);
            data10=data10*16+d;
        }
        return data10;
    }
    ...
    static QString SetFileDateName()
    {
        QDateTime time=QDateTime::currentDateTime();
        //return time.toString("yyyy年MM月dd日 hh时mm分ss秒 dddd");
